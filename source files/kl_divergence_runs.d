module kl_divergence_runs;


import mimic_looking_for_more_optima_with_kl_divergence;
    ...
    const size_t maximumNumberOfIterations, const size_t numberOfTrials, const string path)
{
    ...
    const dataSeparator = "::";
    file.writeln("success", dataSeparator, "divergences");
    file.close;
    scope(exit)
    {
        file.close;
    ...
    foreach(_; iota(0, numberOfTrials).parallel)
    {
        ...
        synchronized
        {
            ...
            file.writeln(result.succeeded, dataSeparator, result.divergences);
        }
    }
}
...
void main()
{
    ...
    const directoryName = "UBM" ~ dirSeparator ~ "klDivergence";
    const nMin = 50;
    const nMax = 200;
    const nStepSize = 10;
    ...
    foreach(n; iota(nMin, nMax + 1, nStepSize).parallel)
    {
        ...
        const lambda = cast(int) (12 * n * log(cast(double) n));
