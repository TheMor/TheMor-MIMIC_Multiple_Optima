module mimic_looking_for_more_optima;
...
private:
    ...
    Tuple!(bool, "succeeded", Individual[], "bestIndividuals", size_t, "bestIteration", size_t, "numberOfIterations",
...
public:
    ...
    auto run()
    {
        ...
        // Get the best individuals of this iteration.
        Individual[] bestIndividuals;
        auto fitness = &this.fitnessFunction.evaluate;
        const maximumFitness = this.fitnessFunction.optimalFitnessValue();
        foreach(individual; population)
        {
            if(fitness(individual) == maximumFitness)
            {
                bestIndividuals ~= individual.dup;
            }
        }
        ...
        if(this.foundOptimum(population))
        {
            foreach(_; 0 .. currentNumberOfIterations)
            {
                ...
                // Add all optimal individuals found.
                foreach(individual; population)
                {
                    if(fitness(individual) == maximumFitness)
                    {
        ...
        this.returnTuple.bestIndividuals = bestIndividuals;
